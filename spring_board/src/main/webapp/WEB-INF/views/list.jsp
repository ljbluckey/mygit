			...
			<c:otherwise>
				...
				<c:forEach var="boardVO" items="${listModel.boardVOList}">
					<tr>
						<td>${boardVO.seq}</td>
						<td>
							<!-- 답글에 대한 들여쓰기 체크 --> <c:if test="${boardVO.blevel > 0}">
								<!-- 들여쓰기 횟수만큼 "-" 반복 -->
							...
							글번호와 현재 페이지번호(목록으로 되돌아올때 직전 보고 있던 해당 페이지가 노출 )를 전달--> <a
							href='<c:url value="/board_detail.do?seq=${boardVO.seq}&pn=${listModel.requestPage}"/>'>
								${boardVO.title} </a>
						</td>
				...
				<tr>
					...
					<td colspan="5"><c:if test="${beginPage > 10}">
							<!-- 예: 이전 [11][12] "이전"클릭시 10번으로-->
							<a href='<c:url value="/board_list.do?pn=${beginPage-1}"/>'>이전</a>
						</c:if> <!-- 이전과 다음사이의 페이지 번호들 [9][10][11][12]--> <c:forEach var="pno"
